	...
	<div class="card-body card-block">
	    <form id="useredit" method="get" action="{{ url('/user/update') }}"  >
	        ...
	        <div class="row form-group">
	            ...
	            <div class="col-12 col-md-9"><input type="text" id="name" name="name" placeholder="Enter Name..." class="form-control  @error('name') is-invalid @enderror" value="@if(isset($value[0]['name'])) {{$value[0]['name']}} @else{{old('name')}} @endif" required autocomplete="name"></div>
				@error('name')
	        ...
	        <div class="row form-group">
	            ...
	            <div class="col-12 col-md-9">
	            	...
	            	value="@if(isset($value[0]['email'])) {{ $value[0]['email'] }} @else {{old('email')}} @endif" required autocomplete="email">
	            </div>
				@error('email')
                    <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
